...
module.exports.onLoad = function () {
    ...
    const path = join(__dirname, "events", "cache", "leaveGif", "randomgif");
...
module.exports.run = async function ({ args, event, api, Threads, getText }) {
    try {
        ...
        switch (args[0]) {
            ...
            case "mp4": {
                const path = join(__dirname, "events", "cache", "leaceGif", "randomgif");
                ...
                if (msg == "remove") {
                    ...
                    if (!msg.match(/(http(s?):)([/|.|\w|\s|-])*\.(?:gif|mp4)/g)) return api.sendMessage(getText("invaildURL"), threadID, messageID);
                    try {
