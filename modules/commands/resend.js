  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`${name} Khỏi phải che dấu Trẫm đã thấy hết rồi \n👻──── •💜• ────👻\n\nNội dung: ${getMsg.msgBody}\n\n👻──── •💜• ────👻
Nơi nào có Trẫm, nơi đó có công lý`,threadID)
      else {
            let num = 0
            let msg = {
              body:`🔰${name} Khỏi phải che dấu Trẫm đã thấy hết rồi\n👻──── •💜• ────👻\n${getMsg.attachment.length} tệp đính kèm.${(getMsg.msgBody != "") ? `\n\nNội dung: ${getMsg.msgBody}\n\n👻──── •💜• ────👻
Nơi nào có Trẫm, nơi đó có công lý` : ""}`,
              attachment:[],
